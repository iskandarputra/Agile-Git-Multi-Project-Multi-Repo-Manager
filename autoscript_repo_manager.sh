...
WORK_DIR="$HOME/Documents/workspace"           # CUSTOMIZATION BY USER

project_1_config() {
    local PROJECT_DIR=$AUTOSCRIPT_DIR # CUSTOMIZATION BY USER
    setup_project_config "$PROJECT_DIR" \
        "workspace/RM1/repos_management" \
        "workspace/RM2/repos_management" \
        "workspace/RM3/repos_management"
}

project_2_config() {
    local PROJECT_DIR=$WORK_DIR # CUSTOMIZATION BY USER
    setup_project_config "$PROJECT_DIR" \
    ...
    # mkdir -p project_1/repo_1_folder && cd project_1/repo_1_folder && git clone <YOUR_GIT_REPO>
    #
    # TREE STRUCTURE:
    # Documents/
    # └── workspace
...
project_3_config() {
    local PROJECT_DIR=$WORK_DIR # CUSTOMIZATION BY USER
    setup_project_config "$PROJECT_DIR" \
        "project_2/repo_1_folder/your_git_repo" \
        "project_2/repo_2_folder/your_git_repo"
}
    ...
    select selected_project in "${projects[@]}"; do
        ...
        for ((index = 0; index < ${#projects[@]}; index++)); do
            if [ $REPLY -eq $((index + 1)) ]; then
                echo -e "${GREEN}Switching to ${projects[$index]} PROJECT${NC}"
...
if [ -n "$ACTIVE_PROJECT" ]; then
    ...
    if declare -f "$config_function" >/dev/null; then
        ...
        if [ "$print_repo_count" = true ]; then
            echo "Detected Clone Repos: $num_repos"
        fi
    else
